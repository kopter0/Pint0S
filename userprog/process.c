...
process_create_initd (const char *file_name) {
	...
	while (*(file_name + l) != ' ')
			l++;
	char *prog_name = (char*)palloc_get_page(0);
...
duplicate_pte (uint64_t *pte, void *va, void *aux) {
	...
	// pml4_set_page(current->pml4, newpage, parent_page, writable);
...
lazy_load_segment (struct page *page, void *aux) {
	...
	uint8_t kva = (uint8_t)va;
	if (file_read(lsi -> file, kva, lsi -> read_bytes) != (int) lsi -> read_bytes){
		return false;
	}

	memset (kva + read_bytes, 0, lsi -> zero_bytes);
}
